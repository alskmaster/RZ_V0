(function(){
  ...
  window.ModuleCustomizers['cpu_table'] = {
    modal: null,
    elements: {},
    _ensure(){ const el = ensureCpuTbl(); if(!this.modal) this.modal = new bootstrap.Modal(el); this.elements = {
  ...
  function ensureCpuTbl(){ let el=document.getElementById('customizeCpuTblModal'); if(el) return el; const tpl=document.createElement('div'); tpl.innerHTML=`
  <div class="modal fade" id="customizeCpuTblModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><div class="modal-content">
      ...
      <div class="modal-body">
        <div class="mb-3"><label class="form-label" for="cpuTblHostContains">Filtrar hosts (contém)</label><input type="text" class="form-control" id="cpuTblHostContains" placeholder="Parte do nome do host"/></div>
        <div class="row g-3">
          <div class="col-6"><label class="form-label" for="cpuTblSortBy">Ordenar por</label><select id="cpuTblSortBy" class="form-select"><option value="Avg">Avg</option><option value="Max">Max</option><option value="Min">Min</option></select></div>
          <div class="col-6 form-check"><input class="form-check-input" type="checkbox" id="cpuTblSortAsc"/><label class="form-check-label" for="cpuTblSortAsc">Ascendente</label></div>
          <div class="col-6"><label class="form-label" for="cpuTblTopN">Top N</label><input type="number" class="form-control" id="cpuTblTopN" min="0" value="0"/></div>
          <div class="col-6"><label class="form-label" for="cpuTblDecimals">Casas decimais</label><input type="number" class="form-control" id="cpuTblDecimals" min="0" max="4" value="2"/></div>
        </div>
      </div>
  ...
  </div>`; document.body.appendChild(tpl.firstElementChild); return document.getElementById('customizeCpuTblModal'); }
})();
