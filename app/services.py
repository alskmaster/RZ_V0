...
# ImportaÃ§Ã£o dos nossos Plugins (Collectors)
...
class ReportGenerator:
    def __init__(self, config, task_id):
        ...
        if not self.token or not self.url:
            raise ValueError("ConfiguraÃ§Ã£o do Zabbix nÃ£o encontrada ou token invÃ¡lido.")
    ...
    def _get_client_sla_contract(self):
        ...
        - Procura por atributos comuns no objeto Client (futuro-compatÃ­vel).
        - Caso nÃ£o exista, tenta fallback em system_config (chave DEFAULT_SLA_CONTRACT se disponÃ­vel).
            ...
            for attr in ("sla_contract", "sla", "sla_policy", "sla_plan", "sla_goal"):
                ...
                if val is not None:
                    try:
                        ...
                        current_app.logger.debug(f"[ReportGenerator] SLA encontrado em Client.{attr} (nÃ£o numÃ©rico): {val}")
                        return val
        except Exception as e:
    ...
    def generate(self, client, ref_month_str, system_config, author, report_layout_json):
        """Gera o relatÃ³rio com base no layout configurado (JSON)."""
        self.client = client
        self.system_config = system_config
        self.cached_data = {}

        self._update_status("Iniciando geraÃ§Ã£o do relatÃ³rioâ€¦")

        # --- PerÃ­odo de referÃªncia ---
        try:
            ref_date = dt.datetime.strptime(f'{ref_month_str}-01', '%Y-%m-%d')
        except ValueError:
            return None, "Formato de mÃªs de referÃªncia invÃ¡lido. Use YYYY-MM."
        ...
        current_app.logger.debug(f"[ReportGenerator.generate] perÃ­odo={period} ref={ref_month_str}")

        # --- Grupos do cliente (RELACIONAMENTO DINÃ‚MICO) ---
        try:
        ...
        if not group_ids:
            return None, f"O cliente '{client.name}' nÃ£o possui Grupos Zabbix associados."
        ...
        try:
            ...
            current_app.logger.error(f"[ReportGenerator.generate] Layout JSON invÃ¡lido: {e}", exc_info=True)
            return None, "Layout invÃ¡lido (JSON)."
        ...
        # PrÃ©-coleta de disponibilidade (SLA/KPI/Top)
        if any(mod.get('type') in availability_module_types for mod in (report_layout or [])):
            self._update_status("Coletando dados de Disponibilidade (SLA)â€¦")
            ...
            if error_msg:
                current_app.logger.warning(f"[ReportGenerator.generate] Erro SLA primÃ¡rio: {error_msg}")
                final_html_parts.append(f"<p>Erro crÃ­tico ao coletar dados de disponibilidade: {error_msg}</p>")
                availability_data_cache = {}

        # MÃªs anterior para SLA comparativo
        ...
        if sla_module_config and availability_data_cache:
            ...
            if custom_options.get('compare_to_previous_month'):
                self._update_status("Coletando dados do mÃªs anterior para comparaÃ§Ã£o de SLAâ€¦")
                ...
                if prev_error:
                    self._update_status(f"Aviso: Falha ao coletar dados do mÃªs anterior: {prev_error}")
            ...
            try:
                ...
                if module_type in availability_module_types:
                    if availability_data_cache:
                        ...
                        html_part = "<p>Dados de disponibilidade indisponÃ­veis para este mÃ³dulo.</p>"
                else:
                ...
                final_html_parts.append(f"<p>Erro crÃ­tico ao processar mÃ³dulo '{module_type}'.</p>")
        ...
        self._update_status("Montando o relatÃ³rio finalâ€¦")
        ...
        db.session.commit()
        AuditService.log(f"Gerou relatÃ³rio customizado para '{client.name}' referente a {ref_month_str}", user=author)
    ...
    def _collect_availability_data(self, all_hosts, period, sla_goal, trends_only=False):
        ...
        # Itens de PING com triggers (necessÃ¡rios para correlacionar eventos de indisponibilidade)
        ...
        try:
            for _c in list(df_sla.columns):
                if ('Tempo' in str(_c)) and ('Indispon' in str(_c)) and _c != 'Tempo Indisponível':
                    df_sla.rename(columns={_c: 'Tempo Indisponível'}, inplace=True)
        except Exception:
            pass
        ...
        # Compatibilidade com coletores: garante colunas 'Problema' e 'Ocorrências'
        try:
            import pandas as _pd
            if isinstance(df_top_incidents, _pd.DataFrame):
                if 'Problemas' in df_top_incidents.columns and 'Ocorrências' not in df_top_incidents.columns:
                    df_top_incidents = df_top_incidents.rename(columns={'Problemas': 'Ocorrências'})
                if 'Problema' not in df_top_incidents.columns and 'Ocorrências' in df_top_incidents.columns:
        ...
        self._update_status("Calculando tendÃªncias de KPIsâ€¦")
    ...
    def get_hosts(self, groupids):
        ...
        self._update_status("Coletando dados de hostsâ€¦")
        body = {
            'jsonrpc': '2.0',
            'method': 'host.get',
            'params': {
    ...
    def shared_collect_latency_and_loss(self, all_hosts, period):
        ...
        if df_lat.empty and df_loss.empty:
            return None, "Nenhum item de LatÃªncia ('icmppingsec') ou Perda ('icmppingloss') encontrado."
    ...
    # -------------------- Itens e SÃ©ries (Trends/History) --------------------
    def get_items(self, hostids, filter_key, search_by_key=False, exact_key_search=False, include_triggers=False):
        ...
        """
        self._update_status(f"Buscando itens com filtro '{filter_key}'â€¦")
        params = {
    ...
    def get_trends(self, itemids, time_from=None, time_till=None):
        ...
        if time_from is None or time_till is None:
            raise TypeError("get_trends() requer time_from e time_till, ou um dict 'period' como 2Âº argumento.")
        self._update_status(f"Buscando tendÃªncias para {len(itemids)} itensâ€¦")
        body = {
            'jsonrpc': '2.0',
            'method': 'trend.get',
            'params': {
        ...
        if not isinstance(trends, list):
            current_app.logger.error(f"Falha ao buscar trends para {len(itemids)} itens. Resposta invÃ¡lida do Zabbix.")
            return []
        return trends
    ...
    def obter_eventos(self, object_ids, periodo, id_type='hostids', max_depth=3):
        ...
        if max_depth <= 0:
            current_app.logger.error("ERRO: Limite de profundidade de recursÃ£o atingido para obter eventos.")
            return None
        params = {
        ...
        if isinstance(resposta, dict) and 'error' in resposta:
            self._update_status("Consulta pesada detectada, quebrando o perÃ­odoâ€¦")
    ...
    def obter_eventos_wrapper(self, object_ids, periodo, id_type='objectids'):
        if not object_ids:
            return []
        self._update_status(f"Processando eventos para {len(object_ids)} objetos em uma Ãºnica chamadaâ€¦")
        all_events = self.obter_eventos(object_ids, periodo, id_type)
        if all_events is None:
            current_app.logger.critical("Falha crÃ­tica ao coletar eventos para os IDs. Abortando.")
            return None
        return sorted(all_events, key=lambda x: int(x['clock']))

    # -------------------- Helpers de SLA (correlaÃ§Ã£o e agregaÃ§Ãµes) --------------------
    def _correlate_problems(self, problems, all_events, period=None):
        """
        Pareia eventos de problema (value='1') com o primeiro evento subsequente de recuperaÃ§Ã£o (value='0')
        do mesmo trigger (objectid). Se nÃ£o houver recuperaÃ§Ã£o, encerra no fim do perÃ­odo consultado.
        ...
        for p in problems:
            try:
                ...
                if end_clock is None:
                    # Se nÃ£o tiver recuperaÃ§Ã£o, usa o fim do perÃ­odo (se fornecido) ou o Ãºltimo evento
    ...
    def _calculate_sla(self, correlated_problems, hosts_for_sla, period):
        ...
        Retorna lista de dicts com Host, SLA (%) e Tempo IndisponÃ­vel (horas).
        """
        if not hosts_for_sla:
            return []
        ...
        for hid, down in downtime_by_host.items():
            ...
            rows.append({
                ...
                'SLA (%)': float(sla),
                'Tempo IndisponÃ­vel': round(down / 3600.0, 2)
            })
        return rows

    def _count_problems_by_host(self, problems, all_hosts):
        """
        Conta eventos de problema por host e retorna DataFrame (ou lista compatÃ­vel) ordenado.
        """
        import pandas as pd
