...
# A importaimportacao foi dividida em duas para buscar cada funcao de seu arquivo de origem correto.
...
def before_request_func():
    ...
    if not cfg:
        # Cria um registro padrao para evitar falhas em templates que esperam g.sys_config
        try:
            cfg = SystemConfig()
            db.session.add(cfg)
            db.session.commit()
    ...
    with app_context:
        try:
            ...
            if not all([system_config, client, author]):
                update_status(task_id, "Erro: Dados invalidos.")
                return
            ...
            if error:
                ...
                with TASK_LOCK:
                    ...
                    REPORT_GENERATION_TASKS[task_id]['status'] = "Concluido"
        except Exception as e:
            error_trace = traceback.format_exc()
            current_app.logger.error(f"Erro fatal na thread (Task ID: {task_id}):\n{error_trace}")
            update_status(task_id, "Erro: Falha critica durante a geracao.")

# --- Rotas Principais do Usuario ---
...
def report_status(task_id):
    with TASK_LOCK:
        task = REPORT_GENERATION_TASKS.get(task_id, {'status': 'Tarefa nao encontrada.'})
    return jsonify(task)
...
def download_final_report(task_id):
    ...
    if not task or 'file_path' not in task:
        flash("Arquivo do relatorio nao encontrado ou a tarefa expirou.", "danger")
        return redirect(url_for('main.gerar_form'))
    ...
    if os.path.exists(absolute_path):
        ...
        current_app.logger.error(f"Tentativa de download falhou. Caminho nao encontrado: {absolute_path}")
        flash("Arquivo do relatorio nao existe mais no servidor.", "danger")
        return redirect(url_for('main.gerar_form'))
...
def history():
    ...
    return render_template('history.html', title="Historico", reports=reports)
...
def download_report(report_id):
    ...
    AuditService.log(f"Re-download do relatorio '{report.filename}'")
    ...
    if os.path.exists(absolute_path):
        ...
        current_app.logger.error(f"Tentativa de download do historico falhou. Caminho nao encontrado: {absolute_path}")
        flash("Arquivo de relatorio do historico nao encontrado no servidor.", "danger")
        return redirect(url_for('main.history'))
...
def delete_report(report_id):
    ...
    if not is_authorized:
        flash("Acesso negado. Voce nao tem permissao para excluir este relatorio.", "danger")
        return redirect(url_for('main.history'))
    ...
    try:
        ...
        if os.path.exists(absolute_path):
            ...
            current_app.logger.info(f"Arquivo '{report.file_path}' excluido com sucesso.")
        else:
            current_app.logger.warning(f"Tentativa de excluir arquivo que nao existe: {absolute_path}")

        db.session.delete(report)
        db.session.commit()
        
        AuditService.log(f"Relatorio '{report.filename}' excluido por {current_user.username}")
        flash(f"Relatorio '{report.filename}' exclui­do com sucesso.", "success")

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Erro ao tentar excluir o relatorio {report.id}: {str(e)}")
        flash("Ocorreu um erro ao excluir o relatorio.", "danger")
...
def get_templates():
    ...
    for t in templates:
        ...
        # aplica migracao apenas na visualizacao
        try:
            import json
...
def get_available_modules(client_id):
    ...
    # CORRECAO: atributo correto é 'group_id' e precisamos .all() para materializar
    group_ids = [g.group_id for g in client.zabbix_groups.all()]
    
    config_zabbix, erro = obter_config_e_token_zabbix(current_app.config)
    if erro:
        current_app.logger.error(f"Falha ao obter modulos para client_id {client_id}: {erro}")
...
def search_items(client_id):
    ...
    # CORRECAO: atributo correto é 'group_id' e precisamos .all()
    group_ids = [g.group_id for g in client.zabbix_groups.all()]
...
# --- ROTA DE TESTE DE VALIDACAO (TEMPORARIA) ---
@main.route('/test_events/<int:client_id>/<string:mes_ref>')
@login_required
def test_events(client_id, mes_ref):
    """
    Rota de diagnostico para validar a coleta de eventos.
    Compara a coleta em lote (mas inteiro) com a coleta iterativa (dia a dia).
    """
    client = db.session.get(Client, client_id)
    if not client:
    ...
    try:
        ...
        return jsonify({"erro": "Formato de data invalido. Use YYYY-MM"}), 400

    config_zabbix, erro = obter_config_e_token_zabbix(current_app.config)
    if erro:
        return jsonify({"erro": f"Falha ao conectar ao Zabbix: {erro}"}), 500

    # CORRECAO: 'group_id' + materializacao com .all()
    group_ids = [g.group_id for g in client.zabbix_groups.all()]
    ...
    # --- Coleta A: Metodo em Lote (Atual) ---
    periodo_lote = {
        'start': int(start_date.timestamp()),
        'end': int(end_date.replace(hour=23, minute=59, second=59).timestamp())
    }
    
    # Reutiliza a funcao de obter eventos do ReportGenerator
