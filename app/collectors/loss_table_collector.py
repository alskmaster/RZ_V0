...
class LossTableCollector(BaseCollector):
    """
    Perda de Pacotes (Tabela): apresenta Min/Avg/Max por host com filtros e resumo.
    ...
    def collect(self, all_hosts, period):
        self._update_status("Coletando dados de Perda de Pacotes (tabela)...")
        ...
        if cache_key not in self.generator.cached_data:
            ...
            if error:
                return self.render('loss_table', {'table_html': f"<p><i>{error}</i></p>", 'summary_text': None})
            self.generator.cached_data[cache_key] = data
        df = self.generator.cached_data[cache_key].get('df_loss', pd.DataFrame())
        if df is None or df.empty:
            return self.render('loss_table', {'table_html': '<p><i>Sem dados de perda de pacotes para o período selecionado.</i></p>', 'summary_text': None})
        ...
        if show_summary:
            ...
            summary = f"Tabela de Perda de Pacotes por host (Min/Avg/Max), {ord_txt}. Período: {per_txt}. Itens exibidos: {items}."

        return self.render('loss_table', {'table_html': html, 'summary_text': summary})