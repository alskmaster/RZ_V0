...
class LatencyChartCollector(BaseCollector):
    """
    Latência (Gráficos): Min/Avg/Max com filtros, período e tipos (Barras/Pizza).
    ...
    def collect(self, all_hosts, period):
        ...
        if cache_key not in self.generator.cached_data:
            ...
            if error:
                return self.render('latency_chart', {'img': None, 'summary_text': None})
        ...
        if chart_type == 'pie':
            ...
            img = generate_multi_bar_chart(
                df, 'Latência Média (ms)', 'Latência (ms)', colors,
        ...
        if show_summary:
            ...
            summary = (
                f"Visualização de Latência por host em {tipo}. "
                f"Considera estatísticas de Mínimo, Médio e Máximo. "
                f"Período: {per_txt}. Itens exibidos: {total_hosts}."
            )

        return self.render('latency_chart', {'img': img, 'summary_text': summary})