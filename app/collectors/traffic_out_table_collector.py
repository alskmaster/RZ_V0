# app/collectors/traffic_out_table_collector.py
...
class TrafficOutTableCollector(BaseCollector):
    """Modulo Trafego de Saida (Tabela) com filtros, ordenacao e resumo."""

    DEFAULT_TITLE = 'Trafego de Saida (Tabela)'
    ...
    def _format_summary(self, period, sort_by, sort_asc, total_rows, top_n, interfaces):
        ...
        summary = (
            f"Tabela de Trafego de Saida por host (Minimo/Medio/Maximo em Mbps), {order_txt}. "
    ...
    def collect(self, all_hosts, period):
        ...
        if not table_html:
            table_html = '<p><i>Tabela de Trafego de Saida indisponivel.</i></p>'

        if not self.module_config.get('title'):
            self.module_config['title'] = self.DEFAULT_TITLE

        return self.render('traffic_out_table', {
