...
class LossChartCollector(BaseCollector):
    """
    Perda de Pacotes (Gráficos): Min/Avg/Max com filtros, período e tipos (Barras/Pizza).
    ...
    def collect(self, all_hosts, period):
        self._update_status("Coletando dados de Perda de Pacotes (gráficos)...")
        ...
        if cache_key not in self.generator.cached_data:
            ...
            if error:
                return self.render('loss_chart', {'img': None, 'summary_text': None})
        ...
        if chart_type == 'pie':
            ...
            img = generate_multi_bar_chart(
                df, 'Perda de Pacotes Média (%)', 'Perda (%)', colors,
        ...
        if show_summary:
            ...
            summary = (
                f"Visualização de Perda de Pacotes por host em {tipo}. "
                f"Considera estatísticas de Mínimo, Médio e Máximo. "
                f"Período: {per_txt}. Itens exibidos: {total_hosts}."
            )

        return self.render('loss_chart', {'img': img, 'summary_text': summary})