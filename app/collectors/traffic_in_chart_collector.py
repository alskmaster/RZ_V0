# app/collectors/traffic_in_chart_collector.py
import base64
...
class TrafficInChartCollector(BaseCollector):
    """Modulo Trafego de Entrada (Graficos) com filtros e opcoes visuais."""

    DEFAULT_TITLE = 'Trafego de Entrada (Graficos)'
    ...
    def _build_summary(self, period, chart_type, item_count, top_n, interfaces):
        ...
        summary = (
            f"Grafico de Trafego de Entrada por host ({chart_names.get(chart_type, 'Barras')}). "
    ...
    def collect(self, all_hosts, period):
        ...
        colors = [
            options.get('color_max') or '#ffc266',
            options.get('color_avg') or '#ffa31a',
            options.get('color_min') or '#e68a00',
        ]
        ...
        return self.render('traffic_in_chart', {
            'img': image,
